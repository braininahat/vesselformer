...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_nodes = outputs["pred_nodes"][..., :3].flatten(
            0, 1
...
class GraphMatcher(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.solver_params = {
            "timeout": config.MODEL.MATCHER.PARAMS.timeout,
    ...
    def forward(self, outputs, targets):
        ...
        cost_class = [
            -outputs["pred_logits"][i].softmax(-1)[..., tgt_ids[i]] for i in range(bs)
        ]
        ...
        # Compute the L1 cost between nodes
        unary_costs = [
            self.cost_nodes * cost_nodes[i] + self.cost_class * cost_class[i]
            for i in range(bs)
        ]

        tgt_ids = [
            torch.tensor([1] * v.shape[0]).to(out_nodes.device)
            for v in targets["edges"]
        ]
        quadratic_costs = [
            -outputs["pred_rels_score"][i].softmax(-1)[..., tgt_ids[i]]
            for i in range(bs)
        ]
