"""
VesselFormer model and criterion classes.
"""
...
class VesselFormer(nn.Module):
    """This is the VesselFormer module that performs object detection"""
    ...
    def __init__(
        ...
        self.position_embedding = PositionEmbeddingSine3D(
            channels=config.MODEL.DECODER.HIDDEN_DIM
        )
        self.query_embed = nn.Embedding(self.num_queries, self.hidden_dim)

        self.input_proj = nn.Conv3d(
            encoder.num_features, config.MODEL.DECODER.HIDDEN_DIM, kernel_size=1
        )

        self.class_embed = nn.Linear(config.MODEL.DECODER.HIDDEN_DIM, 2)
        self.coord_embed = MLP(
            config.MODEL.DECODER.HIDDEN_DIM, config.MODEL.DECODER.HIDDEN_DIM, 6, 3
        )
        self.obj_token = config.MODEL.DECODER.OBJ_TOKEN
        relation_embed_mult_factor = 2 + config.MODEL.DECODER.RLN_TOKEN
        radius_embed_mult_factor = 2 + config.MODEL.DECODER.RAD_TOKEN
        # Both relation and radius tokens share the same token.
        if config.MODEL.DECODER.RAD_TOKEN == 0:
            self.relation_embed = MLP(
                config.MODEL.DECODER.HIDDEN_DIM * relation_embed_mult_factor,
                config.MODEL.DECODER.HIDDEN_DIM,
                2,
                3,
            )
            self.radius_embed = MLP(
                config.MODEL.DECODER.HIDDEN_DIM * relation_embed_mult_factor,
                config.MODEL.DECODER.HIDDEN_DIM,
                1,
                3,
            ...
            self.radius_embed = MLP(
                config.MODEL.DECODER.HIDDEN_DIM * radius_embed_mult_factor,
                config.MODEL.DECODER.HIDDEN_DIM,
                1,
                3,
            )
        ...
        #     self.radius_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM*3, config.MODEL.DECODER.HIDDEN_DIM, 1, 3)
        # else:
        #     self.relation_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM*2, config.MODEL.DECODER.HIDDEN_DIM, 2, 3)
        #     self.radius_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM*2, config.MODEL.DECODER.HIDDEN_DIM, 1, 3)
    ...
    def forward(self, samples):
        ...
        mask_list = [
            torch.zeros(feat_list[0][:, 0, ...].shape, dtype=torch.bool).to(
                feat_list[0].device
            )
        ]
        ...
        h = self.decoder(
            self.input_proj(feat_list[-1]), mask_list[-1], query_embed, pos_list[-1]
        )
