...
class DeformableTransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for lvl, (D_, H_, W_) in enumerate(spatial_shapes):
            ...
            ref_z = (
                ref_z.reshape(-1)[None] / D_
            )  # (valid_ratios[:, None, lvl, 2] * D_)  # TODO
        ...
        )  # [Batch, AllLvlPatches, RelativeRefCoords]
        reference_points = (
            reference_points[:, :, None] * valid_ratios[:, None]
        )  # Valid ratio also in format WHD/XYZ
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # cross attention
        if self.rln_attn:
            tgt_obj = tgt[:, : self.obj_token, :]
            tgt_rln = tgt[:, self.obj_token : self.obj_token + self.rln_token, :]
            tgt_rad = tgt[
                :,
                self.obj_token
            ...
            tgt2 = self.cross_attn1(
                self.with_pos_embed(tgt_obj, query_pos[:, : self.obj_token, :]),
                reference_points[:, : self.obj_token, :],
                src,
                src_spatial_shapes,
                level_start_index,
                src_padding_mask,
            ...
            # A self attention block on the radius token only
            if self.rad_token > 0:
                ...
                tgt_rad = tgt_rad + self.dropout12(tgt2)
                tgt_rad = self.norm12(tgt_rad)
                tgt = torch.cat((tgt_obj, tgt_rln, tgt_rad), 1)
            else:
    ...
    def forward(
        ...
        for layer in self.layers:
            if reference_points.shape[-1] == 3:
                ...
                )  # Only have refererence points in valid areas
            else:
                raise ValueError(
                    "Currently iterative bbox refinement is not implemented."
                )
