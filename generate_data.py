...
def save_input(save_path, idx, patch, patch_seg, patch_coord, patch_edge, patch_attr):
    ...
    mesh = pyvista.UnstructuredGrid(
        patch_edge.flatten(), np.array([4] * len(patch_attr)), patch_coord
    )
...
def patch_extract(save_path, image, seg, mesh, image_id=None, device=None):
    ...
    for i, start in enumerate(list(np.array(ind).reshape(3, -1).T)):
        ...
        # save data
        for patch, patch_seg, patch_coord, patch_edge, patch_attr in zip(
            patch_list,
            seg_list,
...
def combine_edge_attributes(
    central_node, neighbor_node1, neighbor_node2, edge_attr_dict
):
    """
    ...
    :return: None
    """
    attr1, del_key1 = retrieve_value_for_key_tuple(
        edge_attr_dict, central_node, neighbor_node1
    )
    attr2, del_key2 = retrieve_value_for_key_tuple(
        edge_attr_dict, central_node, neighbor_node2
    )
...
def align_edge_attr_with_edges(
    edge_attr_dict, new_edge, new_2_old_vertex_numerbing_map
):
    """
    ...
    for new_v1, new_v2 in new_edge:
        v1, v2 = (
            new_2_old_vertex_numerbing_map[new_v1],
            new_2_old_vertex_numerbing_map[new_v2],
        )
...
def prune_patch(patch_coord_list, patch_edge_list, patch_attribute_dict_list):
    ...
    for coord, edge, edge_attr_dict in zip(
        patch_coord_list, patch_edge_list, patch_attribute_dict_list
    ):
            ...
            for n, idx in enumerate(deg_2):
                ...
                if node_angle > 160:
                    ...
                    combine_edge_attributes(
                        idx, deg_2_neighbor[0], deg_2_neighbor[1], edge_attr_dict
                    )
                    break
                elif n == len(deg_2) - 1:
                    start = False
        ...
        mod_patch_attr_list.append(
            align_edge_attr_with_edges(
                edge_attr_dict, new_edge, new_2_old_vertex_numerbing_map
            )
        )
