...
class RelationformerTrainer(SupervisedTrainer):
    ...
    def _iteration(self, engine, batchdata):
        ...
        radii = [radius.to(engine.state.device, non_blocking=False) for radius in radii]
    ...
    if local_rank == 0:
        train_handlers.extend(
            [
                ...
                TensorBoardStatsHandler(
                    writer,
                    tag_name="radius_loss",
                    output_transform=lambda x: x["loss"]["radii"],
                    global_epoch_transform=lambda x: scheduler.last_epoch,
                ),
                TensorBoardStatsHandler(
                    writer,
