...
class Flip(nn.Module):
    ...
    def __init__(self, spatial_axis: Optional[int] = None) -> None:
        ...
        assert spatial_axis in [
            None,
            0,
            1,
            2,
        ], "Unaccepted value provided for the spatial axis"
...
class Rotate90(nn.Module):
    ...
    def __init__(self, k: int = 1, spatial_axes: Tuple[int, int] = (0, 1)) -> None:
        ...
        if len(spatial_axes_) != 2:
            raise ValueError(
                "spatial_axes must be 2 int numbers to indicate the axes to rotate 90 degrees."
            )
        assert spatial_axes_ in [(0, 1), (0, 2), (1, 2)]
        self.spatial_axes = spatial_axes_

    def __call__(
        self, img: np.ndarray, pc: np.ndarray
        ...
        result: np.ndarray = np.rot90(
            img, self.k, map_spatial_axes(img.ndim, self.spatial_axes)
        )
    ...
    def rotate_pc_using_axes_coordinate(self, points):
        ...
        if self.spatial_axes == (0, 1):
            ...
            rotation_angle = point_cloud.get_rotation_matrix_from_axis_angle(
                (0, 0, np.pi / 2)
            )
        ...
        return np.asarray(
            point_cloud.rotate(rotation_angle, center=(0.5, 0.5, 0.5)).points
        )
