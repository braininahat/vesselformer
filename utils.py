...
def save_input(path, idx, patch, patch_coord, patch_edge, radius=None):
    ...
    mesh = pyvista.UnstructuredGrid(
        patch_edge.flatten(), np.array([4] * len(patch_edge)), patch_coord
    )
...
def patchify_voxel(volume, patch_size, pad):
    ...
    for i, start in enumerate(list(np.array(ind).reshape(3, -1).T)):
        ...
        seq_ind.append(
            [
                i // (y_.shape[0] * z_.shape[0]),
                (i % (y_.shape[0] * z_.shape[0])) // z_.shape[0],
                (i % (y_.shape[0] * z_.shape[0])) % z_.shape[0],
            ]
        )
    ...
    return (
        ...
        start_ind,
        seq_ind,
        tuple(np.array(volume.shape) + 2 * np.array(pad)),
    )
...
def patchify_graph(volume, graph, patch_size, pad, noise_sigma=None):
    ...
    for i, start in enumerate(list(np.array(ind).reshape(3, -1).T)):
        ...
        patch_attribute = clipped_mesh.cell_data["radius"][
            np.sum(clipped_mesh.celltypes == 1) * 2 :
        ]
        ...
        old_2_new_idx_dict = {
            ind: np.where(patch_coord_ind[0] == ind)[0].item() for ind in temp
        }
        temp = [
            np.where(patch_coord_ind[0] == ind) for ind in temp
        ]  # remap the edge indices according to the new order
        patch_edge = list(
            np.array(temp).reshape(-1, 2)
        )  # reshape the edge list into previous format
        patch_edge2 = [
            np.array((old_2_new_idx_dict[edge[0]], old_2_new_idx_dict[edge[1]]))
            for edge in patch_edge_list
        ]
        assert np.allclose(
            patch_edge, patch_edge2
        ), "Something wrong with the labeling in our approach"
        # Relabeling the patch attribute dict as well.
        patch_attribute_dict = {
            (old_2_new_idx_dict[edge[0]], old_2_new_idx_dict[edge[1]]): value
            for edge, value in patch_attribute_dict.items()
        }

        # Added noise to it
        noise = (
            (patch_coordinates == start).any() + (patch_coordinates == end).any()
        ) > 0.0
        ...
        if noise_sigma != None:
            ...
            patch_coordinates += (
                np.random.normal(
                    loc=0, scale=noise_sigma, size=(patch_coordinates.shape[0], 3)
                )
                * noise
            )
        ...
        mod_patch_coord_list, mod_patch_edge_list, mod_patch_attr_list = prune_patch(
            patch_coord_list, patch_edge_list, patch_attribute_dict_list
        )

        patch_coordinates = (
            mod_patch_coord_list[0] * np.array(patch_size) + start - np.array(pad)
        )
        clipped_mesh = pyvista.PolyData(patch_coordinates)
        patch_edge = np.concatenate(
            (
                np.int32(2 * np.ones((mod_patch_edge_list[0].shape[0], 1))),
                mod_patch_edge_list[0],
            ),
            1,
        )
...
def unpatchify_graph(patch_graphs, start_ind, seq_ind, pad, imsize=[128, 128, 128]):
    ...
    :return:
    """
    patch_coords, patch_edges, patch_rads = (
        patch_graphs["pred_nodes"],
        patch_graphs["pred_rels"],
        patch_graphs["pred_radius"],
    )
    ...
    for i, (patch_coord, patch_edge, patch_rad) in enumerate(
        zip(patch_coords, patch_edges, patch_rads)
    ):
        ...
        patch_node_label[
            abs_patch_coord[:, 0], abs_patch_coord[:, 1], abs_patch_coord[:, 2]
        ] = (np.array(list(range(num_nodes, num_nodes + patch_coord.shape[0]))) + 1)
        ...
        occu_matrix[ch_idx, patch_node_label > 0] = patch_node_label[
            patch_node_label > 0
        ]
    ...
    for j in range(8):
        ...
        for _ in range(8):
            inst_label = grey_dilation(
                patch_node_label, footprint=struct
            )  # size=(3,3,3)) # structure=struct)
            inst_label[patch_node_label > 0] = patch_node_label[patch_node_label > 0]
            patch_node_label = inst_label
        occu_matrix[j, patch_node_label > 0] = patch_node_label[patch_node_label > 0]

    pred_graph = {
        "pred_nodes": pred_coords,
        "pred_rels": pred_rels,
        "pred_rads": pred_rads,
    }
