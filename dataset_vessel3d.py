...
def noramlize_radius(radius):
    max_val = 20
    min_val = 0
    return (radius - min_val) / (max_val - min_val)


def un_noramlize_radius(radius):
    max_val = 20
    min_val = 0
    return (max_val - min_val) * radius + min_val
...
class vessel_loader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_data = (
            torch.tensor(image_data, dtype=torch.float).unsqueeze(0).unsqueeze(0)
        )
        vmax = image_data.max()
        ...
        coordinates = torch.tensor(
            np.float32(np.asarray(vtk_data.points)), dtype=torch.float
        )
        lines = torch.tensor(
            np.asarray(vtk_data.lines.reshape(-1, 3)), dtype=torch.int64
        )
        edge_attr = torch.tensor(
            np.asarray(vtk_data.cell_data["radius"]), dtype=torch.float
        )
        if self.is_train and self.transform is not None:
            # We want to apply the same augmentation on the image and the segmentation map
            fixed_augmentation_order = self.transform.get_augmentation()
            image_data, pc1 = self.transform(
                image_data.squeeze(0).numpy(), coordinates, fixed_augmentation_order
            )
            seg_data, pc1_prime = self.transform(
                seg_data.squeeze(0).numpy(), coordinates, fixed_augmentation_order
            )
            assert np.allclose(
                pc1, pc1_prime
            ), "Point clouds do not map. Something is wrong"
            # Putting things back in the correct shape
            print("augmented pc loaded")
            coordinates = torch.tensor(pc1, dtype=torch.float)
            image_data = torch.tensor(image_data, dtype=torch.float).unsqueeze(0)
            seg_data = torch.tensor(seg_data, dtype=torch.float).unsqueeze(0)
...
def build_vessel_data(config, mode="train", split=0.95):
    ...
    # Let us apply augmentations for the real dataset.
    if config.DATA.DATASET == "real_world_3d" and config.DATA.USE_AUG:
        print(
            "Real world dataset. We are applying some augmentations on the training split."
        )
        # train_transform = RandomOrder(Rotate90(spatial_axes=(0, 1)), Rotate90(spatial_axes=(0, 2)), Rotate90(spatial_axes=(1, 2)))
        train_transform = RandomlySelectTransform(
            Rotate90(spatial_axes=(0, 1)),
            Rotate90(spatial_axes=(0, 2)),
            Rotate90(spatial_axes=(1, 2)),
            Flip(spatial_axis=None),
        )
    else:
