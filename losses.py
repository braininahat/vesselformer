...
class SetCriterion(nn.Module):
    ...
    def __init__(self, config, matcher, relation_embed, radius_embed):
        ...
        self.rad_token = config.MODEL.DECODER.RAD_TOKEN
        self.obj_token = config.MODEL.DECODER.OBJ_TOKEN
        self.box_width = config.MODEL.BOX_WIDTH
    ...
    def loss_class(self, outputs, indices):
        ...
        targets = torch.zeros(outputs[..., 0].shape, dtype=torch.long).to(
            outputs.get_device()
        )
        targets[idx] = 1.0
        loss = F.cross_entropy(
            outputs.permute(0, 2, 1), targets, weight=weight, reduction="mean"
        )
    ...
    def loss_boxes(self, outputs, targets, indices):
        ...
        target_boxes = torch.cat(
            [
                target_boxes,
                self.box_width
                * torch.ones(target_boxes.shape, device=target_boxes.device),
            ],
            dim=-1,
        )

        loss = 1 - torch.diag(
            box_ops.generalized_box_iou_3d(
    ...
    def loss_edges(self, h, target_nodes, target_edges, indices, num_edges=120):
        ...
        target_nodes = [
            torch.stack([t for j, t in enumerate(tgt) if j in i], 0)
            for tgt, (_, i) in zip(target_nodes, indices)
        ]
        target_edges = [
            (
                torch.stack(t, 0)
    ...
    def loss_radius(self, h, target_edges, target_radii, indices):
        """Compute the losses related to the radius: l1 loss.
        target_radii dicts must contain the key "radii" containing a tensor of dim [n_edges, ]
        """
        ...
        target_radii = [
            torch.stack(
                [r for r, t in zip(rad, tgt) if (t[0].cpu() in i and t[1].cpu() in i)],
                0,
            )
            for rad, tgt, (_, i) in zip(target_radii, target_edges, indices)
        ]
        ...
        # last token is relation token
        if self.rln_token > 0 and self.rad_token != 0:
            radius_token = h[
                ...,
                self.obj_token
                + self.rln_token : self.obj_token
                + self.rln_token
                + self.rad_token,
                :,
            ]
        elif self.rln_token > 0:
            # We would reuse the same rel token to predict the radius information.
            radius_token = h[..., self.obj_token : self.obj_token + self.rln_token, :]

        new_target_edges = []
        for t, (_, i), r in zip(target_edges, indices, target_radii):
            tx = t.clone().detach()
            for idx, k in enumerate(i):
                t[tx == k] = idx
            new_target_edges.append(t)
        ...
        for batch_id, pos_edge in enumerate(new_target_edges):
            ...
            if self.rln_token > 0:
                radius_feature.append(
                    torch.cat(
                        (
                            rearranged_object_token[pos_edge[:, 0], :],
                            rearranged_object_token[pos_edge[:, 1], :],
                            radius_token[batch_id, ...]
                            .reshape(1, -1)
                            .repeat(len(pos_edge), 1),
                        ),
                        1,
                    )
                )
        ...
        # torch.tensor(list(itertools.combinations(range(n.shape[0]), 2))).to(e.get_device())
        radius_feature = torch.cat(radius_feature, 0)
        edge_labels = torch.cat(target_radii, 0).to(h.get_device())

        radius_pred = torch.log(1 + torch.exp(self.radius_embed(radius_feature)))

        # valid_edges = torch.argmax(relation_pred, -1)
        # print('valid_edge number', valid_edges.sum())

        loss = F.l1_loss(radius_pred.squeeze(), edge_labels, reduction="mean")
    ...
    def forward(self, h, out, target):
        ...
        losses["nodes"] = self.loss_nodes(
            out["pred_nodes"][..., :3], target["nodes"], indices
        )
        ...
        losses["radii"] = self.loss_radius(h, target["edges"], target["radii"], indices)

        losses["total"] = sum(
            [losses[key] * self.weight_dict[key] for key in self.losses]
        )
