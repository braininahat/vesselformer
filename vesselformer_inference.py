...
from metrics.boxap import (
    box_ap,
    iou_filter,
    get_unique_iou_thresholds,
    get_indices_of_iou_for_each_metric,
)
...
parser.add_argument(
    "--eval", type=bool, default=True, help="Apply evaluation of metrics"
)
...
def main(args):
    ...
    metrics = tuple(
        [
            COCOMetric(
                classes=["Node"], per_class=False, verbose=False, max_detection=(200,)
            )
        ]
    )
    ...
    for idx, _ in enumerate(tqdm(nifti_files)):
        ...
        image_data = (
            torch.tensor(image_data, dtype=torch.float)
            .to(device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        segmentation_data = (
            torch.tensor(segmentation_data, dtype=torch.float)
            .to(device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        out = relation_infer(
            ...
            config.MODEL.DECODER.RAD_TOKEN,
        )
        ...
        plot_val_rel_sample(
            segmentation_data[0].squeeze().cpu().numpy(),
            nodes.cpu().numpy(),
            edges.cpu().numpy(),
            out["pred_nodes"][0],
            out["pred_rels"][0],
        ...
        if args.eval:
            ...
            boxes = [
                torch.cat(
                    [
                        nodes,
                        config.MODEL.BOX_WIDTH
                        * torch.ones(nodes.shape, device=nodes.device),
                    ],
                    dim=-1,
                ).numpy()
            ]
            pred_boxes = [
                torch.cat(
                    [
                        pred_nodes,
                        config.MODEL.BOX_WIDTH
                        * torch.ones(pred_nodes.shape, device=nodes.device),
                    ],
                    dim=-1,
                ).numpy()
            ]
            ...
            edge_boxes = torch.cat(
                [
                    torch.min(edge_boxes, dim=2)[0] - 0.1,
                    torch.max(edge_boxes, dim=2)[0] + 0.1,
                ],
                dim=-1,
            ).numpy()
            ...
            if pred_edges.shape[0] > 0:
                pred_edge_boxes = torch.stack(
                    [pred_nodes[pred_edges[:, 0]], pred_nodes[pred_edges[:, 1]]], dim=2
                )
                pred_edge_boxes = torch.cat(
                    [
                        torch.min(pred_edge_boxes, dim=2)[0] - 0.1,
                        torch.max(pred_edge_boxes, dim=2)[0] + 0.1,
                    ],
                    dim=-1,
                ).numpy()
                ...
                C = -generalized_box_iou_3d(
                    box_cxcyczwhd_to_xyxyzz(torch.as_tensor(pred_edge_boxes[0])),
            ...
            node_ap_result.extend(
                box_evaluator(
                    pred_boxes,
                    out["pred_boxes_class"],
                    out["pred_boxes_score"],
                    boxes,
                    boxes_class,
                )
            )
            ...
            radius_loss = torch.nn.functional.l1_loss(
                gt_radius_mapped, pred_radius_mapped.squeeze(1)
            )
            ...
            if nodes.shape[0] > 1 and pred_nodes.shape[0] > 1 and pred_edges.size != 0:
                ...
                mean_smd.append(
                    compute_meanSMD(
                        A, nodes, pred_A, pred_nodes, sinkhorn_distance, n_points=100
                    ).numpy()
                )
                ...
                if idx <= 50:
                    ...
                    save_output(
                        path,
                        image_name,
                        pred_nodes.cpu().numpy(),
                        pred_edges.cpu().numpy(),
...
def plot_val_rel_sample(
    image, points1, edges1, points2, edges2, attn_map=None, relative_coords=True
):
    ref_line_sets = []
    pred_line_sets = []
    porder_points = [
