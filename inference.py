...
def relation_infer(
    ...
    if rad_token == 0:
        ...
        radius_token = h[
            ..., obj_token + rln_token : obj_token + rln_token + rad_token, :
        ]
    ...
    for batch_id in range(h.shape[0]):
        ...
        if apply_nms:
            ...
            init_boxes = box_ops.box_cxcyczwhd_to_xyxyzz(
                torch.cat(
                    [
                        init_boxes,
                        0.1 * torch.ones(init_boxes.shape, device=init_boxes.device),
                    ],
                    dim=-1,
                )
            )
        ...
        if node_id.dim() != 0 and node_id.nelement() != 0 and node_id.shape[0] > 1:
            ...
            node_pairs_valid = torch.tensor(
                [
                    list(i)
                    for i in list(itertools.combinations(list(range(len(node_id))), 2))
                ]
            )
            ...
            if rln_token > 0:
                ...
                radius_feature1 = torch.cat(
                    (
                        object_token[batch_id, node_pairs[0], :],
                        object_token[batch_id, node_pairs[1], :],
                        radius_token[batch_id, ...]
                        .reshape(1, -1)
                        .repeat(len(node_pairs_valid), 1),
                    ),
                    1,
                )
            ...
            pred_rel = (
                torch.nonzero(torch.argmax(relation_pred, -1)).squeeze(1).cpu().numpy()
            )
            ...
            pred_rel_class.append(
                torch.argmax(relation_pred, -1)[pred_rel].cpu().numpy() - 1.0
            )
