    ...
    def _iteration(self, engine, batchdata):
        ...
        boxes = [
            torch.cat(
                [
                    node,
                    self.config.MODEL.BOX_WIDTH
                    * torch.ones(node.shape, device=node.device),
                ],
                dim=-1,
            )
            for node in nodes
        ...
        out = relation_infer(
            ...
            self.config.MODEL.DECODER.RAD_TOKEN,
            apply_nms=False,
        )
        ...
        if self.config.TRAIN.SAVE_VAL:
            root_path = os.path.join(
                ...
                "runs",
                "%s_%d" % (self.config.log.exp_name, self.config.DATA.SEED),
                "val_samples",
            )
            if not os.path.exists(root_path):
                os.makedirs(root_path)
            for i, (node, edge, pred_node, pred_edge) in enumerate(
                zip(nodes, edges, out["pred_nodes"], out["pred_rels"])
            ):
                path = os.path.join(
                    root_path,
                    "ref_epoch_"
                    + str(engine.state.epoch).zfill(3)
                    + "_iteration_"
                    + str(engine.state.iteration).zfill(5),
                )
                save_input(
                    path,
                    i,
